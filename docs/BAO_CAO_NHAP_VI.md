...
Repo đã có script để chuyển dữ liệu ảnh được gán nhãn sang format YOLO chuẩn. Các lớp đề xuất cho fine-tune gồm:
...
### 12.2 Train temporal model

Từ `metadata.jsonl`, hệ thống trích xuất các feature theo thời gian như:
...
- baseline temporal training scaffold
...
- chưa đánh giá định lượng đầy đủ trên tập dữ liệu đủ lớn
...
- huấn luyện temporal model với nhiều clip hơn
